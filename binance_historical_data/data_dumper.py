...
class BinanceDataDumper:
    ...
    def __init__(
        ...
        if save_format not in self._SAVE_FORMAT_ENUM:
            raise ValueError(
                f"Unknown save format: {save_format} not in {self._SAVE_FORMAT_ENUM}"
            )
    ...
    def _download_raw_file(self, str_url_path_to_file, str_path_where_to_save):
        ...
        while retry_count < max_retries:
            try:
                ...
                LOGGER.debug(
                    "[WARNING] URL Error: %s for %s", ex.reason, str_url_path_to_file
                )
                retry_count += 1
    ...
    def _download_raw_file_v2(
        self, str_url_path_to_file: str, str_path_where_to_save: str
    ):
            ...
            try:
                if "trades" not in key.lower():
                    self._s3.download_file(
                        self._bucket_name, key, str_path_where_to_save
                    )
                else:
                    with tqdm(
                        unit="B",
